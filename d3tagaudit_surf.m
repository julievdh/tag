function     RES = d3tagaudit(tag,tcue,RES,S,fhigh,upperplot,lowerplot)
...
%     specified deployment tag at start mcue tcue and using a pre-existing
%     audit structure R.
%
...
if nargin<5 | isempty(fhigh),
    fhigh = DEFAULT_FHIGH;
end

if nargin<6 | isempty(upperplot)
    ...
    while done == 0,
        ...
        elseif button=='i',
            if all(current>0),
                ...
                try
                    cd '\\uni.au.dk\Users\au575532\Documents\MATLAB\BreathSounds\' % added for JvdH's Machine
                    wavwrite(x_temp,afs_org,16,'tempsound.wav'); %
                    % wavwrite has been removed, try audiowrite(FILENAME,Y,FS)
                    % audiowrite('C:\tempsound.wav',x_temp,afs);
                catch
